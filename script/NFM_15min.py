...
class NMF_Net:
    def __init__(self, batch_size, hidden_factor, layers, loss_type, features_M,
    ...
    def _init_graph(self):
        ...
        self.train_features = tf.placeholder(tf.int32, shape=[None, None],
                                             name="input_train_feature")  # None * features_M
        self.train_labels = tf.placeholder(tf.float32, shape=[None, 1], name="input_train_label")  # None * 1
        self.dropout_keep = tf.placeholder(tf.float32, shape=[None], name="input_dropout")
        self.train_phase = tf.placeholder(tf.bool, name="input_phase")
        ...
        self.out = tf.add_n([Bilinear, self.Feature_bias, Bias], name="output")  # None * 1
        # print(self.out)
        # Compute the loss.
        ...
        with tf.name_scope("gradients"):
            if self.optimizer_type == 'AdamOptimizer':
                ...
                self.opt = tf.keras.optimizers.SGD(lr=self.learning_rate,
                                                   momentum=0.0,
                                                   decay=0.0,
                                                   nesterov=False)
                self.optimizer = self.opt.minimize(self.loss)
