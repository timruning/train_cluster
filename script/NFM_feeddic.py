...
class NMF_Net:
    def __init__(self, batch_size, hidden_factor, layers, loss_type, features_M, random_seed, batch_norm,
    ...
    def _init_graph(self):
        ...
        test_feature = tf.constant([[100000]] * self.batch_size, dtype=tf.int32)

        self.embedding7 = tf.nn.embedding_lookup(self.weights['feature_embeddings'], test_feature,
                                                 name="test_embedding7")

        tf.summary.histogram(name="test_embedding", values=self.embedding7)
        ...
        with tf.name_scope("gradients"):
            if self.optimizer_type == 'AdamOptimizer':
                ...
                # self.grads = self.opt.compute_gradients(loss=self.loss, var_list=self.params)
                ...
                # self.grads = self.opt.compute_gradients(loss=self.loss, var_list=self.weights['prediction'])
                self.optimizer = self.opt.minimize(self.loss)
    ...
    def _initialize_weights(self):
        ...
        if num_layer > 0:
            ...
            tf.summary.histogram("weight_prediction", values=all_weights['prediction'])
        else:
